...
class ManualNeuralNetwork:
    ...
    def __init__(self, input_size, hidden_layers, output_size=1, learning_rate=0.001):
        ...
        self.hidden_layers = hidden_layers  # [64, 32] gibi
    ...
    def forward(self, X):
        ...
        output = z_output  # Linear activation for regression
    ...
    def fit(self, X, y, epochs=100, verbose=False):
        for epoch in range(epochs):
            ...
            # Compute loss
            loss = np.mean((output.flatten() - y.flatten()) ** 2)
...
class GeneticAlgorithm:
    """
    Sequence-based Genetic Algorithm with Manual Neural Network
    ...
    def __init__(self, generation_size, crossover_p, mutation_p, sequence_length, 
                 hidden_layers=[64, 32], learning_rate=0.001, generations=20):
    ...
    def train_model(self, X_sequences, y, chromosome):
        """Train manual neural network with chromosome-weighted sequences"""
        try:
            # Apply chromosome weights to sequences
            X_weighted = self.apply_chromosome_weights(X_sequences, chromosome)
            
            # Flatten sequences for neural network input
            ...
            # Create and train manual neural network
            input_size = X_train.shape[1]
            model = ManualNeuralNetwork(
    ...
    def run(self, X_sequences, y):
        ...
        print(f"Starting Genetic Algorithm with Manual Neural Network:")
    ...
    def plot_chromosome_weights(self):
        ...
        plt.tight_layout()
        plt.savefig('manual_nn_chromosome_weights.png', dpi=150, bbox_inches='tight')
        plt.show()
        print("Chromosome weights plot saved as 'manual_nn_chromosome_weights.png'")


def main():
    """Main function to run the sequence-based genetic algorithm with manual NN"""
    print("=== Sequence-based GA with Manual Neural Network Implementation ===")
    
    # Define sequence length
    sequence_length = 60  # Your sequence length
    
    # Initialize GA with manual neural network
    ga = GeneticAlgorithm(
        generation_size=15,
        crossover_p=0.8,
        mutation_p=0.2,
    ...
    print(f"Chromosome length: {sequence_length}")
    print(f"Input to NN: {sequence_length} * {X_selected.shape[1]} = {sequence_length * X_selected.shape[1]} neurons")

    # Run genetic algorithm
    best_chromosome = ga.run(X_sequences, y_sequences)
    
    # Evaluate best model
    print("\nEvaluating best model with manual neural network...")
